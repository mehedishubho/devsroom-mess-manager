...
**Last activity:** 2026-09-14 - Completed quick task 260915-1ab: DEPLOYMENT.md restructured into a serial, path-first setup guide for public use — 12 sections (§1 pick path, §2 prerequisites, §3 one `.env` checklist for every path, §4–§7 four self-contained deployment paths [Forge / manual VPS / Docker panels / cPanel] each containing its own queue-worker + scheduler setup, §8 post-install, §9 verification, §10 monitoring, §11 one merged troubleshooting table with a Path column, §12 backup & restore runbook appendix). Env tables consolidated (old §5 + §12.2), troubleshooting merged (old §10 + §11.9 + §12.5), all internal § refs renumbered (0 dead of 28) + external citations re-pointed in README.md / SETUP-USAGE-DEPLOY.md / .env.example. Docs-only, zero content loss. 2 commits (7e9eee4, 5cf8b1a). — Previous: 2026-09-14 - Completed quick task 260914-wm1: Backup system usability pass — easier to configure (guided Google Drive OAuth connect that stores the refresh token for you and creates the folder, Test-connection against typed values so you don't save-then-test, one-click presets, inline provider setup help, plain-language config summary) and easier to manage (one System health panel covering scheduler/queue/last-backup/mirrors with a fix command per row, per-destination storage + growth, purge preview sharing one rotation rule with the command, per-run duration + archive size). Also fixed a real bug: `BackupConfig::current()` memoized its fallback default, so a transient early read made the whole process use default settings and duplicate the singleton row. 5 commits (28c1ced, 4895fba, 87c6321, 0be8805, 2ed4c4c). No test runner in repo — verified with migrate + view:cache + route:list + pint + tinker smokes. Deploy: `php artisan migrate`. — Previous: 2026-09-14 quick task 260914-uk4: Backups dashboard (/dashboard/backups) overhaul — removed the DigitalOcean Spaces destination system entirely and fixed/added everything on the page (purge logging, all-destination delete, monitor-noise control, log audit + retention, stats header, per-destination ticks, checksums + Verify, search/sort/paging/bulk delete, filterable+exportable activity log with fix hints, UI-configurable alerts + archive encryption, clearable secrets, pre-restore safety backup, upload-restore, queued backup/restore with in-place progress, maintenance-mode recovery). 6 commits on master (f044870, 6025954, 5c7187e, 7aea69c, a4b6214, ceb2593). NOTE the queue worker is required for UI-triggered backup/restore. — Previous: 2026-08-03 quick task 260803-mbr: Fixed member login "ACCESS DENIED" — RootController sent every authenticated user to admin `/dashboard`, so the normal login flow (visit `/` → login → intended `/`) stranded members on a 403 with no logout. RootController now routes by role (member→/my). Redirect defect, not a missing role. Commit ef50dc1 (code-only, no migration).
...
| 260724-pm2 | Dashboard Total meals + Credit/Dues cards; removed restore-test subsystem entirely; removed the whole PHPUnit tests/ folder + phpunit.xml + phpunit/mockery dev deps; backup scheduler-health banner + scheduled-run activity logging; README full feature guide + balance explainer | 2026-07-24 | 3f30535 | [260724-pm2-dashboard-cards-remove-restore-test-back](./quick/260724-pm2-dashboard-cards-remove-restore-test-back/) |
| 260801-szi | Expenses & Payments lists scoped by month (current-month default + This month / Specific month / Whole year / All time selector; From/To removed from Payments, filter form added to Expenses; pagination kept); Monthly Report split into Brought forward / This month / Closing (net) so a carried-forward advance no longer reads as this-month credit; additive `brought_forward` column frozen at month-close (bill-preview cache re-keyed v1→v2). Settlement/carry-forward engine untouched. | 2026-08-02 | 684736d | [260801-szi-expenses-and-payments-month-scoping-with](./quick/260801-szi-expenses-and-payments-month-scoping-with/) |
| 260803-mbr | Member login "ACCESS DENIED" + stuck — RootController redirected every authed user to admin `/dashboard`; the normal login-via-`/` flow (guest stores `url.intended=/`) bypassed PostLoginRedirectController, so members landed on the admin panel and 403'd. Fixed RootController to route by role (member→/my, manager→/home, super-admin→/dashboard+onboarding, role-less→logout). Redirect defect, NOT a missing role. | 2026-08-03 | ef50dc1 | [260803-mbr-fix-member-login-403-root-redirect](./quick/260803-mbr-fix-member-login-403-root-redirect/) |
| 260914-uk4 | Backups dashboard overhaul — removed the DigitalOcean Spaces destination system entirely; fixed purge-never-logged, local-only delete (cloud copies survived), nightly monitor spam, unaudited log deletion, unbounded `backup_logs` (+`backup:prune-logs`), unclearable saved secrets, dead `enabled_spaces`; added stats header, per-destination ticks, sha256 + Verify, search/sort/paging/bulk delete, filterable+exportable activity log with fix hints, UI-configurable notification email + AES-256 archive encryption, pre-restore safety backup, upload-restore, queued backup/restore with in-place progress, maintenance-mode recovery. 6 commits. Deploy: `php artisan migrate` + `config:clear` + `view:cache`; **queue worker now required for UI backup/restore**. | 2026-09-14 | ceb2593 | [260914-uk4-backups-dashboard-overhaul-fix-gaps-add-](./quick/260914-uk4-backups-dashboard-overhaul-fix-gaps-add-/) |
| 260914-wm1 | Backup usability — guided **Connect Google Drive** OAuth (no hand-built refresh token; auto-creates the folder), Test connection against typed values (no save-first), one-click presets, inline provider setup help, plain-language config summary; single **System health** panel (scheduler/queue/last backup/mirrors with the fix command each), per-destination storage + growth vs 7d, purge preview sharing `BackupRetention::plan()` with the command, per-run duration + archive size in the activity log. Also fixed `BackupConfig::current()` memoizing its fallback default (process-wide default settings + duplicate config rows). 5 commits. Deploy: `php artisan migrate`. | 2026-09-14 | 2ed4c4c | [260914-wm1-backup-system-usability-easier-to-config](./quick/260914-wm1-backup-system-usability-easier-to-config/) |
| 260915-1ab | DEPLOYMENT.md restructured into a serial, path-first setup guide for public use — 12 sections (§1 pick path, §2 prerequisites, §3 one `.env` checklist for every path, §4–§7 four self-contained deployment paths [Forge / manual VPS / Dokploy+Coolify / cPanel] each containing its own queue-worker + scheduler setup, §8 post-install, §9 verification, §10 monitoring, §11 one merged troubleshooting table with a Path column, §12 backup & restore runbook appendix). Env tables consolidated (old §5 + §12.2), troubleshooting merged (old §10 + §11.9 + §12.5), all internal § refs renumbered (28 unique refs, 0 dead) and external citations re-pointed in README.md / SETUP-USAGE-DEPLOY.md / .env.example. Docs-only, zero content loss (verbatim Dockerfile/compose/nginx/supervisor/cron blocks intact). 2 commits (7e9eee4, 5cf8b1a). | 2026-09-14 | 7e9eee4 | [260915-1ab-rearrange-deployment-md-into-a-clear-ser](./quick/260915-1ab-rearrange-deployment-md-into-a-clear-ser/) |
| 260915-4yg | Committed the three Docker deployment files that DEPLOYMENT.md §6.1 documents but that had never existed in the repo — `Dockerfile` (nginx+PHP-FPM 8.4 supervised, node:24 Vite stage, mariadb-client, RUN_MIGRATIONS/OPTIMIZE entrypoint gates), `docker-compose.yml` (4 services: app/queue/scheduler/db=MariaDB 11.4, no host ports, env from panel UI), `.dockerignore` (13 lines). Extracted byte-exact from the committed doc via sed line-ranges (diff-verified ALL empty; 13/88/83 lines; 0 literal secrets; 0 ports). Unblocks Dokploy compose builds; the scheduler service now ships with the compose. 1 commit (fb37bf9). | 2026-09-14 | fb37bf9 | [260915-4yg-commit-the-three-missing-docker-deployme](./quick/260915-4yg-commit-the-three-missing-docker-deployme/) |
