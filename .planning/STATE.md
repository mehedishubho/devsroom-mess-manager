...
**Last activity:** 2026-09-14 - Completed quick task 260914-uk4: Backups dashboard (/dashboard/backups) overhaul — removed the DigitalOcean Spaces destination system entirely and fixed/added everything on the page (purge logging, all-destination delete, monitor-noise control, log audit + retention, stats header, per-destination ticks, checksums + Verify, search/sort/paging/bulk delete, filterable+exportable activity log with fix hints, UI-configurable alerts + archive encryption, clearable secrets, pre-restore safety backup, upload-restore, queued backup/restore with in-place progress, maintenance-mode recovery). 6 commits on master (f044870, 6025954, 5c7187e, 7aea69c, a4b6214, ceb2593). No test runner exists (removed in 260724-pm2) — verified with migrate + view:cache + route:list + schedule:list + pint + tinker smokes. Deploy: `php artisan migrate` + `config:clear` + `view:cache`; NOTE the queue worker is now required for UI-triggered backup/restore. — Previous: 2026-08-03 quick task 260803-mbr: Fixed member login "ACCESS DENIED" — RootController sent every authenticated user to admin `/dashboard`, so the normal login flow (visit `/` → login → intended `/`) stranded members on a 403 with no logout. RootController now routes by role (member→/my). Redirect defect, not a missing role. Commit ef50dc1 (code-only, no migration). — Previous: 2026-08-02 quick task 260801-szi: Expenses & Payments lists scoped by month (current-month default + This month / Specific month / Whole year / All time selector; From/To removed from Payments, filter form added to Expenses); Monthly Report split into Brought forward / This month / Closing (net) so a member's prior-month advance is no longer misread as this-month credit (fixes the "Rakib Credit ৳1,985 with no August payment" confusion); additive `brought_forward` column frozen at month-close + bill-preview cache re-keyed v1→v2. Settlement/carry-forward engine untouched — the close still carries real balances forward, now clearly labelled. 3 commits on master (6beaf5a, b12ff75, 684736d). Deploy: `php artisan migrate` + `config:clear` + `view:clear`. NOTE: repo has no PHPUnit/Pest runner (removed in 260724-pm2); verification used view:cache + migrate + pint --test + grep + tinker gates.
