...
**The Backups page:**
- **System health** — one panel for Scheduler, Queue worker, Last backup and Off-site mirrors. Anything unhealthy shows the exact command to fix it, so there is no guessing why backups aren't happening.
- **Storage & rotation** — per-destination usage (count / size / oldest / newest, plus reachable or not), local usage against the configured cap, growth vs a week ago, and a preview of exactly which archives the next cleanup will delete.
...
- **Nobody is left guessing.** "Backups configured but nothing is happening" is turned into a named cause by the System health panel — missing cron (with the exact line to install and a Copy button), an idle queue worker, a stale last-backup, or an unreachable mirror — each paired with the command that fixes it.
...
- **Configuration** (inline) — start from a **preset** (Light / Balanced / Maximum safety), then adjust schedule (frequency + time), retention (keep-days + storage cap), the per-provider toggles, the backup-notification email, optional AES-256 archive encryption, and explicit **remove saved secret** controls. Each provider carries its own "How do I get these?" setup steps.
- **Connect Google Drive** — runs Google's consent flow and stores the refresh token (and creates the destination folder) for you, instead of making you hand-build a token. **Test connection** checks the values currently in the form, so you don't have to save first.
