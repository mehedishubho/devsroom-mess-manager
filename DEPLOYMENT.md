...
Production hardening checklist for shipping the v1 pilot. Covers the Laravel Forge path (primary — faster to ship), self-hosted Docker panels ([Dokploy](https://dokploy.com/) / [Coolify](https://coolify.io/) — §6), cPanel shared hosting (§7), and a manual VPS path (§5). The v1 pilot is **one mess, one monthly cycle** — a single small VPS is plenty.

> **Bare shared hosting is RULED OUT for the worker.** The month-close runs as a queued job (`app/Jobs/CloseMonthJob.php`) and requires a persistent queue worker via `supervisor` (or a container-native worker service — §6). Shared hosts cannot reliably run a persistent worker (the worker is killed on every request boundary / cron tick / panel restart), and there is no way to recover from that class of failure without shell access. **You need a VPS** — or shared hosting with `QUEUE_CONNECTION=sync` and its trade-offs accepted (§7). A $5/month DigitalOcean/Hetzner droplet is sufficient for the pilot.

---

## 1. Pick your deployment path
...
| **Dokploy / Coolify (Path C, §6)** | You want Forge-style UX (git-push deploys, TLS, logs, rollbacks) without the Forge bill — both panels are free/open-source and run on your own VPS. You commit a Dockerfile + compose file to the repo; the panel builds and runs web + queue + scheduler + MySQL. | ~$5–10/mo VPS only |
| **cPanel shared hosting (Path D, §7)** | Cheapest possible single-mess install; jobs run inline (`sync`), month-close blocks the browser for a few seconds. | Existing shared-hosting plan |
| **Manual VPS (Path B, §5)** | You want full control / lower cost / already have a VPS. | ~$5/mo VPS only |

> **How to read this guide (serial order).** Follow **§2** and **§3** once — they apply to every path. Then do **only your path's section**: §4 (Forge), §5 (manual VPS), §6 (Docker panels), or §7 (cPanel) — the queue worker + scheduler setup is inside each path. Finish with **§8–§10** (post-install essentials, verification, monitoring). **§11** is the troubleshooting table for when something breaks; **§12** is the backup & restore runbook you only open when disaster strikes.
...
The Dokploy / Coolify path requires none of that on the host — the panel runs everything in Docker. You only commit the three files from §6.1.
...
## 3. Prepare the production `.env` — one checklist for every path

Get these wrong and the pilot fails. The first two are non-negotiable security gates (CONCERNS #9 — stack traces leak; T-05-03-01). Fill this in **once**, before you deploy — every path (§4–§7) consumes it.
...
| **`APP_DEBUG`** | **`false`** | **CRITICAL.** If `true`, any error leaks full stack traces + env vars to the browser. (T-05-03-01) On Docker panels the compose file defaults it to `false`. |
| `APP_URL` | `https://your-domain.com` | HTTPS scheme, not HTTP. Used for signed routes + asset URLs; also builds the Google Drive OAuth redirect URI (§12.6). |
| `APP_TIMEZONE` | `Asia/Dhaka` | Carry-forward from Plan 01 D-21. Don't ship UTC. |
| `APP_KEY` | (generated by `php artisan key:generate`) | Never copy dev's key. (Docker panels: run `php artisan key:generate --show` locally and paste the base64 key into the panel.) |
| `DB_CONNECTION` | `mysql` | NEVER sqlite (dev/prod parity constraint). |
| `DB_HOST` / `DB_PORT` / `DB_DATABASE` / `DB_USERNAME` / `DB_PASSWORD` | (production MySQL creds — NOT the dev password) | T-05-03-04: set fresh at deploy time, do not copy dev `.env`. (Docker panels: the same three `DB_*` values also create the MariaDB database/user — the compose file maps them to `MARIADB_*`.) |
| `QUEUE_CONNECTION` | `database` | A persistent worker runs the jobs — supervisor (§4.2 / §5.3) or a container worker service (§6.1 compose). Without this, `CloseMonthJob` never runs (T-05-03-05). Shared-hosting exception: `sync` (§7). |
| `CACHE_STORE` | `database` | Single-mess pilot: database cache is fine. (array/file also acceptable.) |
| `SESSION_DRIVER` | `database` | |
| `MAIL_MAILER` / `MAIL_HOST` / `MAIL_PORT` / `MAIL_USERNAME` / `MAIL_PASSWORD` / `MAIL_FROM_ADDRESS` | your SMTP relay | Needed for invites, password resets + backup-failure email (§12.7). The `log` default swallows mail — see §8.2. |
...
| `BACKUP_NOTIFICATION_EMAIL` | `ops@your-domain.com` | Where spatie sends failure emails (an email saved on the Backups page wins over this). Requires `MAIL_MAILER=smtp` (see §12.7). |
...
> **Strongly recommended on Docker panels (§6):** local backup zips live in the `app-storage` volume, which dies with the VPS — set these keys AND add an off-site mirror from the Backups page (§12.6).

**Backup destinations need no env keys.** Local is always on; Google Drive and Cloudflare R2 are optional mirrors enabled + configured from the Backups page (`/dashboard/backups`) and stored encrypted in the database. See §12.6.

### 3.3 Docker panels only (Dokploy / Coolify)

On Dokploy/Coolify you set the variables once in the panel's **Environment** tab (Dokploy: service → **Environment**; Coolify: resource → **Environment Variables**) — they are interpolated into the compose file (§6.1); nothing else to configure. Everything from §3.1–§3.2 is required there too, PLUS these keys that only exist in the container setup:
...
## 4. Path A — Laravel Forge (primary — recommended for the pilot)

Laravel Forge provisions the VPS for you and writes the supervisor config + cron entry automatically — this section replaces the manual steps in §5.
...
4. In **Environment**, set the production `.env` (see §3 for the full checklist — the hard requirements are `APP_DEBUG=false`, `APP_ENV=production`, `APP_URL=https://...`, MySQL creds, `QUEUE_CONNECTION=database`).

### 4.2 Add the Queue Worker (Forge does this for you)
...
### 4.3 Add the Scheduler (Forge does this for you)
1. Site → **Scheduler** → add `php /home/forge/your-domain.com/artisan schedule:run` at `* * * * *`. Forge writes the crontab entry.

### 4.4 HTTPS (and storage)
1. Site → **SSL** → **Let's Encrypt** (one click). Forge provisions + auto-renews.
2. Force HTTPS redirect stays on (Forge default).
3. If you serve assets from a CDN, set `ASSET_URL` to the CDN's HTTPS URL.

Storage permissions are handled automatically by Forge — no `chown`/`chmod` needed (contrast §5.6).

---

## 5. Path B — Manual VPS

If you are not using Forge, you write the Nginx site, supervisor config, and cron yourself. This section is the do-it-yourself equivalent of Forge's automation (§4).
...
### 5.3 Supervisor config for the queue worker (verbatim)
...
### 5.4 The schedule:run cron (verbatim)
...
> **The cron runs backups; the WORKER runs on-demand ones.** "Backup now", every restore, and the post-close hook are queued jobs. Without the queue worker from §5.3 running, they sit in the `jobs` table and the Backups page shows them as **stuck** (with a red banner). On a container platform with no host cron (Dokploy/Coolify/§6), run `php artisan schedule:work` as a long-lived service instead of this crontab line — and keep a separate `queue:work` service (§6.5).
>
...
`sudo certbot --nginx -d your-domain.com -d www.your-domain.com`. Certbot rewrites the Nginx config to redirect HTTP → HTTPS.

### 5.6 Storage permissions
...
## 6. Path C — Docker panels: Dokploy & Coolify
...
What changes vs. Forge: the app ships as a **container image**, so the repo carries a `Dockerfile` + `docker-compose.yml` instead of relying on a panel-written supervisor config. The compose file runs **four services**: `app` (nginx + PHP-FPM), `queue` (the worker), `scheduler` (Laravel's `schedule:work`), and `db` (MariaDB). Everything in §3 (the `.env` hard requirements) applies unchanged — only the process management differs (see §6.5).

### 6.1 Commit these three files to the repo root

The panels build from your cloned repo, so the files must be committed (not pasted). Copy them verbatim; they encode this project's specifics — PHP 8.4, the required extensions, `mariadb-client` for the spatie backup dumps (§12.1), `mysqldump`-on-`/usr/bin`, 512M `memory_limit` for the PDF/Excel exports (§11), and the exact worker command (§5.3).
...
**`docker-compose.yml`** — the four services. Only the `<<` YAML anchors are structural; every `${VAR}` is filled from the panel's environment UI (§3.3), so secrets never live in the file:
...
4. **Environment** tab: add the §3 variables (§3.3 covers the Docker-only ones).
...
7. **First run**: visit `https://your-domain` → the one-time **`/setup` wizard** creates the super-admin (§8.1). CLI alternative: Dokploy's **Advanced → Run Command** (shell into the `app` container) → `php artisan mess:create-super-admin admin@yourdomain.com "Admin Name" --password=...`.
8. **Verify** with the §9 checklist. Worker/scheduler health = the `queue`/`scheduler` services showing running in the panel; `queue:failed` via Run Command.
...
3. **Environment Variables**: add the §3 variables (§3.3 covers the Docker-only ones).
4. **Deploy** (first build takes a few minutes). The `app` service reports healthy via the compose healthcheck (the `/up` healthcheck defined in §6.1).
...
6. **First run**: visit `https://your-domain` → the `/setup` wizard (§8.1). For artisan one-offs use the resource's **Terminal** tab.
...
8. **Verify** with the §9 checklist (worker = `queue` service running; `php artisan queue:failed` in the Terminal).
...
### 6.5 Worker + scheduler in containers

The compose file already runs the two background services for you:

- `queue` — `php artisan queue:work database --sleep=3 --tries=3 --max-time=3600`, the same command as the supervisor unit in §5.3.
- `scheduler` — `php artisan schedule:work`, which fires `schedule:run` every minute: no host cron needed in a container. This is the container-native replacement for the §5.4 crontab line — run `schedule:work` as a long-lived service instead of cron, and keep a separate `queue:work` service.

Optional extras: Dokploy's **Schedules** UI (service → Schedules) can run `php artisan schedule:run` on a `* * * * *` cron as an alternative to the `scheduler` service; Dokploy's built-in **database backup** on the `db` service (drop to S3) is a fine second copy alongside the app's spatie pipeline (§12).

---

## 7. Path D — cPanel (shared hosting)
...
- All §3 `.env` gates (`APP_DEBUG=false`, fresh DB creds, `APP_TIMEZONE=Asia/Dhaka`, SMTP) apply identically.
...
## 8. Post-install essentials (every path)

### 8.1 Create the first super-admin

Log in as the production admin — the super-admin you set up here (**NOT** `manager@demo.test`, that's dev only). If no super-admin exists yet, visit the app root — the **one-time setup wizard** at `/setup` will guide you through creating the first super-admin account. Alternatively, create one via CLI:
...
`.env.example` ships with `MAIL_MAILER=log`, which writes emails to a log file instead of sending them. For real email — invites, password resets, backup-failure notices — set the `MAIL_*` block to a real SMTP relay per §3.1 (AWS SES, Postmark, SendGrid, or your VPS's mail relay). The backup-failure wiring (`BACKUP_NOTIFICATION_EMAIL`, per-channel routing) is covered in §12.7.
...
2. Log in as the super-admin created in §8.1.
...
6. Verify the queue worker is running:
    ...
    ```
    (Docker panels: the `queue` service showing running in the panel is the equivalent.)
7. Verify the scheduler is running:
    ...
    ```
    (Docker panels: the `scheduler` service replaces this; cPanel: the cron you added in §7.)
...
## 11. Troubleshooting (all paths)

One table for every deployment path. The **Path** column says where each symptom applies. Backup-specific entries come from the §12 runbook; container-specific ones from the Docker panels (§6).

| Symptom | Likely cause | Fix | Path |
|---------|--------------|-----|------|
| Month-close never completes | Queue worker not running | Manual VPS: `sudo supervisorctl status` → if not RUNNING, `sudo supervisorctl start mess-worker:*` (T-05-03-05). Dokploy/Coolify: panel → services → restart `queue`, and check its logs for `CloseMonthJob` exceptions. | All |
| Stack traces visible in browser | `APP_DEBUG=true` shipped | Set `APP_DEBUG=false`, `php artisan config:cache`, redeploy (T-05-03-01) | All |
| `/home` data stale after a write | Cache invalidation not firing | Check `AppServiceProvider::registerBillPreviewInvalidation` listeners; check `CACHE_STORE=database` and that the `cache` table exists | All |
| "MONTH CLOSED" on a legitimate write | The (year, month) is closed — by design | Use `/mess/closings/{closing}/corrections` for adjustment entries (CLOSE-12) | All |
| 500 on PDF/Excel export | Dompdf memory or Excel timeout | Manual VPS: increase PHP `memory_limit` / `max_execution_time` for fpm; check the `php8.4-fpm` pool config. Docker panels: the image sets `memory_limit=512M` — if you added a panel resource limit below ~512 MB, raise it. | All |
| `composer install` tries to load debugbar/telescope | You forgot `--no-dev` | Re-run `composer install --no-dev --optimize-autoloader` | All |
| Assets/links render as `http://` behind Dokploy/Coolify (mixed content) | App is behind the panel's Traefik/Caddy proxy | Set `APP_URL=https://…` and redeploy (Laravel generates URLs from `APP_URL`). If any absolute URLs are still `http`, also trust the proxy: add `$middleware->trustProxies(at: '*')` in `bootstrap/app.php`. | Docker panels |
| `backup:run` fails with `The dump process failed` (dev only) | `mysqldump` not on PATH on Windows dev | Spatie v10 is NOT Windows-compatible (per its requirements doc). Set `DUMP_BINARY_PATH` in dev `.env` to the MySQL bin dir (e.g. `C:\Program Files\MySQL\MySQL Server 8.0\bin`). Prod Linux `/usr/bin/mysqldump` is fine. (Pitfall 2 — see `06-01-SMOKE.md` §4) | Dev only |
| `backup:run` in a Dokploy/Coolify container fails with "dump process failed" | The image is missing a `mysqldump` binary | Use the §6.1 Dockerfile (it installs `mariadb-client`); keep `DUMP_BINARY_PATH=/usr/bin` (default in `.env.example`). | Docker panels |
| A cloud mirror never receives the archive | Invalid credentials, the provider toggle is off, or that mirror disk errored | Open **Backups → Storage providers → Test connection**; it reports the real error. Each backup row also shows a per-destination tick, so a missing tick means that mirror did not receive the archive. | All |
| `backup:monitor` reports `UnhealthyBackupWasFound` | Backup too old OR too big | Check the scheduler is running (`sudo crontab -l \| grep schedule:run`). Check `BACKUP_MAX_MB` — increase if the mess genuinely has more data. Run `php artisan backup:run` manually to refresh. The notification lands in the super-admin bell + email (§12.7). | All |
| Restore shows "Restore failed. App is back online" but the app looks fine | The restore service caught an exception but called `up` in `finally` | Check `storage/logs/laravel.log` for the exception. The audit-log row (`event='backup.restore.failed'`) has the error message under `/mess/audit` (filter `tags=backup`). The live DB was NOT modified if the exception fired before the DB-restore step. | All |
| "Backup now" sits on `running` and then flips to `stuck` | No queue worker is consuming the queue, so the dispatched job never runs | Start one: `php artisan queue:work` (or the `queue` service in your panel / the supervisor program in §5.3). The Backups page shows a red "Background jobs are not being processed" banner while jobs are waiting. | All |
| Restore ran but files 404 on the web (broken image links) | The `public/storage` symlink was clobbered | `php artisan storage:link`. Verify with `ls -la public/storage` → points to `../storage/app/public`. (Pitfall 4 — §12.5 step 6) | All |
| GTID error restoring a managed-MySQL dump (`ERROR 3546`) | `mysqldump` emitted a `SET @@GLOBAL.GTID_PURGED` line (only happens when moving to a managed/hosted MySQL) | Self-managed VPS MySQL (per §2/§5) does NOT hit this. If you later move to a managed MySQL, add `--set-gtid-purged=OFF` to the dump config in `config/database.php` → `connections.mysql.dump`. (Pitfall 10) | Rare (managed MySQL) |
| Month-close completed but no immediate backup landed | Post-close `after()` hook's `Artisan::call('backup:run', ['--only-db' => true])` threw | The hook is wrapped in try/catch so the close itself succeeded (T-06-02-07). Check `storage/logs/laravel.log`. The nightly 01:30 run will still capture the close. | All |
| `config:cache` fails at container start (tyro-login Closure blocker, §12.5 step 7) | A config Closure can't be serialized to the cache | Set `OPTIMIZE=false` in the panel environment — the entrypoint then skips the cache commands (§6.1). | Docker panels |
| `app` scaled to >1 replica → migration errors | Two containers run `migrate --force` concurrently | Keep `app` at **1** replica (Dokploy: Advanced → Cluster). One mess needs one. | Docker panels |
| Panel log viewer is empty | Laravel logs to `storage/logs/laravel.log` (a file), not stdout | Set `LOG_CHANNEL=stderr` to stream into the viewer, or read the file via the panel's terminal. | Docker panels |
| Local backups "disappear" after a redeploy | They live in the `app-storage` volume — volumes persist across redeploys but not across VPS loss | Add an off-site mirror (Google Drive / Cloudflare R2) from the Backups page (§12.6). Optionally add the panel's own DB-backup-to-S3 as a second copy. | Docker panels |

---

## 12. Backup & restore runbook (appendix)
...
**`.env` is deliberately EXCLUDED from backups (D-07).** Secrets must not live in object storage. The consequence: after any restore you MUST regenerate `APP_KEY` + rotate credentials — see §12.5 step 7.

### 12.2 Where backups live
...
- **Off-server mirrors (optional)** — Google Drive and/or Cloudflare R2, enabled + configured from the Backups page (§12.6). Each backup row on the page shows which destinations actually hold the archive.
...
The backup schedule lives in `routes/console.php` and runs on the existing Laravel scheduler (which fires every minute per §5.3/§5.4). The nightly cadence:

| Time | Command | Purpose |
|------|---------|---------|
| 01:00 | `backup:purge` | Prune old backups per the retention ladder (§12.2) across every active destination. |
| (configured) | `backup:run` | The actual DB dump + files zip + mirror upload. `withoutOverlapping()->onOneServer()`. |
| 02:00 | `backup:monitor` | Health-check the latest backup (not too old, not too big). Emits `UnhealthyBackupWasFound` on failure → in-app bell + email (§12.7). |
...
- **Via the UI**: super-admin → `/dashboard/backups` → **Backup now**. This dispatches a queued job, so it never blocks the browser or dies on a gateway timeout. With the production `database` queue it needs the queue worker running (§5.3) — otherwise the run sits in `jobs` until one starts. On shared hosting `QUEUE_CONNECTION=sync` executes it inline instead (blocking, but still recorded in the Activity log). Either way a `running` row appears immediately and is updated in place with the outcome.
...
### 12.4 Restore procedure (PRIMARY path — via the super-admin UI)

Use this path when the app itself is healthy but you need to roll the data back (e.g. a bad migration was applied + reverted, a manager corrupted `monthly_closings` and you need to restore yesterday's snapshot). If the app itself is unreachable, skip to §12.5.
...
### 12.5 Restore procedure (FALLBACK — via CLI, when the UI is unreachable)
...
5. Restore the DB:
   ...
   ```
   (If you moved to DO Managed MySQL, add `--set-gtid-purged=OFF` to the original `mysqldump` config — Pitfall 10. Self-managed VPS MySQL per §2/§5 does NOT hit GTID errors.)
6. Restore files:
   ```bash
   cp -R /tmp/restore-extracted/storage/app/public/. /var/www/mess/storage/app/public/
   php artisan storage:link   # belt-and-suspenders; see Pitfall 4 / §11
   ```
...
### 12.6 Configure off-site mirrors (optional)
...
### 12.7 Enable failure notifications (REQUIRED for prod)
...
### 12.8 Optional: host-level snapshot (defense-in-depth)
...
*Last updated: 2026-09-15. §1–§3 are shared by every path; §4–§7 are the four self-contained deployment paths (queue worker + scheduler live inside each); §9 is the post-deploy smoke test; §11 merges all troubleshooting (previously spread over three tables); §12 is the backup & restore runbook (formerly §11). The supervisor block in §5.3 + the schedule cron in §5.4 remain the verbatim artifacts Forge would write — copy them as-is for manual setups.*
