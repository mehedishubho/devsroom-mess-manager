...
- **Via the UI**: super-admin → `/dashboard/backups` → **Backup now**. This dispatches a queued job, so it never blocks the browser or dies on a gateway timeout. With the production `database` queue it needs the queue worker running (§4) — otherwise the run sits in `jobs` until one starts. On shared hosting `QUEUE_CONNECTION=sync` executes it inline instead (blocking, but still recorded in the Activity log). Either way a `running` row appears immediately and is updated in place with the outcome.
...
7. The restore is **queued** (`RestoreBackupJob`, `$tries = 1`) — a `running` row appears in the Activity log immediately and is updated in place with the outcome. Nothing runs on the request thread (on the production `database` queue this requires the worker from §4; under `QUEUE_CONNECTION=sync` it executes inline instead), so a gateway/PHP timeout cannot hard-kill the restore half-way.
...
- **Per-minute scheduler cron** (cPanel → Cron Jobs): `* * * * * cd /home/USER/mess && /usr/bin/php artisan schedule:run >> /dev/null 2>&1` — this drives backups, `backup:purge`, `backup:monitor`, `backup:prune-logs`, and `telescope:prune`. Without it the backup cadence configured in the Backups UI never fires.
...
- Expect `mysqldump` to already exist (set `DUMP_BINARY_PATH` if `backup:run` fails). No second database is needed — the nightly restore-test was removed in quick task 260724-pm2.
