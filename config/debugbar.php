...
return [
    ...
    // D-06 Layer 2: env-gated; .env.example ships DEBUGBAR_ENABLED=false so prod copies inherit disabled.
    'enabled' => env('DEBUGBAR_ENABLED'),
    'collect_jobs' => env('DEBUGBAR_COLLECT_JOBS', false),
    // D-06 Pitfall 2 + T-05-01-04: never inject the bar into PDF (Dompdf) or API/Excel responses.
    // `except` is the documented URI ignore-list (functionally equivalent to the
    // exclude_paths key referenced in earlier Debugbar releases).
    'except' => [
        'telescope*',
        'horizon*',
        '_boost/browser-logs',
    ...
    */
    ...
    // D-06 Pitfall 2 + T-05-01-05: capture meal-grid AJAX timing (keep true), but
    // never auto-show / re-open the bar on AJAX (auto_show=false) and keep the
    // AJAX handler tab off so the bar can't be injected into JSON response bodies.
    ...
    'ajax_handler_enable_tab' => env('DEBUGBAR_AJAX_HANDLER_ENABLE_TAB', false),
