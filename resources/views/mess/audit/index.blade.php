    ...
    <div class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm md:p-6">
        ...
        <form method="GET" action="{{ route('mess.audit') }}" class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
            ...
            <div class="flex flex-col gap-1">
                ...
                <input type="date" name="from" id="from" value="{{ $filters['from'] ?? '' }}" class="min-h-[44px] w-full rounded-md border border-slate-300 px-3 py-2 text-base text-slate-900 focus:border-emerald-600 focus:outline-none focus:ring-2 focus:ring-emerald-600">
            </div>
            <div class="flex flex-col gap-1">
                <label class="text-sm font-medium text-slate-900" for="to">{{ __('To date') }}</label>
                <input type="date" name="to" id="to" value="{{ $filters['to'] ?? '' }}" class="min-h-[44px] w-full rounded-md border border-slate-300 px-3 py-2 text-base text-slate-900 focus:border-emerald-600 focus:outline-none focus:ring-2 focus:ring-emerald-600">
            </div>
            <div class="flex items-end gap-2 sm:col-span-2 md:col-span-4">
                <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-md bg-emerald-600 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-700 min-h-[44px]">{{ __('Apply filters') }}</button>
                <a href="{{ route('mess.audit') }}" class="inline-flex items-center justify-center gap-2 rounded-md text-slate-700 hover:bg-slate-100 px-3 py-2 text-sm min-h-[44px]">{{ __('Reset') }}</a>
            </div>
        </form>
    </div>
