...
@section('content')
        ...
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-600">
                    {{ __('Member Statement') }} — {{ $statement['period_label'] }}
                    @if ($isSnapshot)
    ...
    {{-- Member + month picker (sticky GET form) --}}
    ...
    @if (empty($row))
        <x-empty-state
            :title="__('No statement available for this member in :period', ['period' => $statement['period_label']])"
            :description="__('The member was not active during this month, or no data was recorded.')" />
    @else
        {{-- Meal-rate math (D-25) --}}
        <section class="mb-6 rounded-xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
            <h2 class="mb-2 text-sm font-semibold uppercase tracking-wider text-emerald-900">{{ __('Meal rate') }}</h2>
        ...
        <section class="mb-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-200 px-5 py-3">
                <h2 class="text-sm font-semibold uppercase tracking-wider text-slate-700">{{ __('Daily meals') }}</h2>
            </div>
            @if (empty($daily))
                <p class="px-5 py-6 text-sm text-slate-500">{{ __('No meal entries for this month.') }}</p>
            @else
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">B</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">L</th>
                                <th class="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-slate-500">D</th>
                                <th class="px-4 py-3 text-right text-xs font-semibold uppercase tracking-wider text-slate-500">{{ __('Meal value') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @foreach ($daily as $d)
                                <tr class="transition-colors hover:bg-slate-50">
                                    <td class="px-4 py-3 text-slate-900">{{ Carbon::parse($d['date'])->format('d-m-Y') }}</td>
                                    <td class="px-4 py-3 text-center">{{ $d['breakfast'] ? '✓' : '—' }}</td>
                                    <td class="px-4 py-3 text-center">{{ $d['lunch'] ? '✓' : '—' }}</td>
                                    <td class="px-4 py-3 text-center">{{ $d['dinner'] ? '✓' : '—' }}</td>
                                    <td class="px-4 py-3 text-right tabular-nums">{{ number_format($d['meal_value'], 1) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot class="bg-slate-50 font-semibold">
                            <tr>
                                <td class="px-4 py-3 text-slate-900">{{ __('Totals') }}</td>
                                <td class="px-4 py-3 text-center tabular-nums">{{ $dailyTotalB }}</td>
                                <td class="px-4 py-3 text-center tabular-nums">{{ $dailyTotalL }}</td>
                                <td class="px-4 py-3 text-center tabular-nums">{{ $dailyTotalD }}</td>
                                <td class="px-4 py-3 text-right tabular-nums">{{ number_format($dailyTotalValue, 1) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
        ...
        @if ($statement['guests']->isNotEmpty())
            <section class="mb-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-5 py-3">
                    <h2 class="text-sm font-semibold uppercase tracking-wider text-slate-700">{{ __('Guest meals') }}</h2>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">{{ __('Date') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">{{ __('Guest') }}</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">{{ __('Meal') }}</th>
                        ...
                        <tfoot class="bg-slate-50 font-semibold">
                            <tr>
                                <td colspan="3" class="px-4 py-3 text-right text-slate-700">{{ __('Total') }}</td>
                                <td class="px-4 py-3 text-right tabular-nums">{{ Money::taka($guestTotal) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
        ...
        <section class="mb-6 grid grid-cols-1 gap-4 md:grid-cols-2">
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-5 py-3">
                    <h2 class="text-sm font-semibold uppercase tracking-wider text-slate-700">{{ __('Bill payments') }}</h2>
                </div>
                @if ($billPayments->isEmpty())
                    <p class="px-5 py-4 text-sm text-slate-500">{{ __('No bill payments this month.') }}</p>
                @else
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-200 text-sm">
                            <thead class="bg-slate-50">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @foreach ($billPayments as $p)
                                    <tr class="transition-colors hover:bg-slate-50">
                                        <td class="px-4 py-3 text-slate-900">{{ $p->date->format('d-m-Y') }}</td>
                                        <td class="px-4 py-3 text-slate-700">{{ __(ucfirst((string) $p->method)) }}</td>
                                        <td class="px-4 py-3 text-slate-600">{{ $p->reference ?? '—' }}</td>
                                        <td class="px-4 py-3 text-right tabular-nums">{{ Money::taka($p->amount) }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                <div class="border-b border-slate-200 px-5 py-3">
                    <h2 class="text-sm font-semibold uppercase tracking-wider text-slate-700">{{ __('Advance deposits') }}</h2>
                </div>
                @if ($advanceDeposits->isEmpty())
                    <p class="px-5 py-4 text-sm text-slate-500">{{ __('No advance deposits this month.') }}</p>
                @else
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-200 text-sm">
                            <thead class="bg-slate-50">
        ...
        <section class="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <dl class="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
                <div>
                    <dt class="text-xs text-slate-500">{{ __('Opening advance') }}</dt>
                    <dd class="mt-1 text-base font-bold text-emerald-700">{{ Money::taka($row['advance_balance'] ?? 0) }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-slate-500">{{ __('Opening due') }}</dt>
                    <dd class="mt-1 text-base font-bold text-rose-700">{{ Money::taka($row['due_balance'] ?? 0) }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-slate-500">{{ __('Fixed share') }}</dt>
                    <dd class="mt-1 text-base font-bold text-slate-900">{{ Money::taka($row['fixed_share'] ?? 0) }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-slate-500">{{ __('Meal cost') }}</dt>
                    <dd class="mt-1 text-base font-bold text-slate-900">{{ Money::taka($row['meal_cost'] ?? 0) }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-slate-500">{{ __('Bill') }}</dt>
                    <dd class="mt-1 text-base font-bold text-slate-900">{{ Money::taka($row['bill'] ?? 0) }}</dd>
                </div>
                <div>
